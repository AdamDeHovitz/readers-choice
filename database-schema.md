...
CREATE INDEX idx_theme_votes_theme_id ON themes_votes(theme_id);
...
CREATE TABLE personal_rankings (
  ...
  rank INTEGER, -- NULL means "not read"
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Anyone can read book clubs they're a member of
...
-- Admins can update their book clubs
...
-- Members can read membership of their book clubs
...
-- Members can read meetings for their book clubs
...
-- Members can read votes for their book club meetings
...
-- Members can read rankings for their book clubs
...
-- Users can manage their own rankings
...
```sql
CREATE OR REPLACE FUNCTION get_year_from_meeting(meeting_id UUID)
RETURNS INTEGER AS $$
  SELECT EXTRACT(YEAR FROM meeting_date)::INTEGER
  FROM meetings
  WHERE id = meeting_id;
$$ LANGUAGE SQL STABLE;
```
...
```sql
-- This will be implemented based on chosen ranking algorithm
-- Placeholder for now
CREATE OR REPLACE FUNCTION calculate_global_rankings(
  p_book_club_id UUID,
  p_year INTEGER
)
RETURNS TABLE (
  ...
  -- Implementation will depend on chosen algorithm (Borda count, etc.)
$$ LANGUAGE SQL STABLE;
```
...
- Allow manual book entry as fallback
